...
logging.basicConfig(filename=f"{sys.argv[3]}-{uid}.log", format='%(asctime)s %(filename)s: %(message)s', filemode='a+')
...
# set eviction and max requests in the orchestrator
  ...
  for strategy in STRATEGIES:
      for benchmark in BENCHMARKS:
          for mutability in MUTABILITIES:
              ...
              deploy_cmd = f"faas-cli deploy --image=skharban/{benchmark} --name={benchmark} --env STRATEGY={strategy} EVICTIONS=True"
              ...
              logger.info("Executing strategy: %s for benchmark %s with mutability %s", strategy, benchmark, mutability)
